...
def main():
    ...
    # DEV read-only QC visibility into what got deployed to prod: the model
    # registry, batch predictions, the scoring task, and the prod feature store.
    # Read-only ONLY (USAGE/SELECT/MONITOR) - no OWNERSHIP/OPERATE/write, so a
    # data scientist can verify a deployment but still cannot deploy or change it.
    run(s, f"GRANT USAGE ON SCHEMA {C.PROD_DATABASE}.{C.REGISTRY_SCHEMA} TO ROLE {C.DEV_ROLE}")
    run(s, f"GRANT USAGE ON ALL MODELS IN SCHEMA {C.PROD_DATABASE}.{C.REGISTRY_SCHEMA} TO ROLE {C.DEV_ROLE}")
    run(s, f"GRANT USAGE ON FUTURE MODELS IN SCHEMA {C.PROD_DATABASE}.{C.REGISTRY_SCHEMA} TO ROLE {C.DEV_ROLE}")
    run(s, f"GRANT USAGE ON SCHEMA {C.PROD_DATABASE}.{C.ANALYTICS_SCHEMA} TO ROLE {C.DEV_ROLE}")
    run(s, f"GRANT SELECT ON ALL TABLES IN SCHEMA {C.PROD_DATABASE}.{C.ANALYTICS_SCHEMA} TO ROLE {C.DEV_ROLE}")
    run(s, f"GRANT SELECT ON FUTURE TABLES IN SCHEMA {C.PROD_DATABASE}.{C.ANALYTICS_SCHEMA} TO ROLE {C.DEV_ROLE}")
    run(s, f"GRANT MONITOR ON ALL TASKS IN SCHEMA {C.PROD_DATABASE}.{C.ANALYTICS_SCHEMA} TO ROLE {C.DEV_ROLE}")
    run(s, f"GRANT MONITOR ON FUTURE TASKS IN SCHEMA {C.PROD_DATABASE}.{C.ANALYTICS_SCHEMA} TO ROLE {C.DEV_ROLE}")
    run(s, f"GRANT USAGE ON SCHEMA {C.PROD_DATABASE}.{C.FEATURE_STORE_SCHEMA} TO ROLE {C.DEV_ROLE}")
    for obj in ("TABLES", "VIEWS", "DYNAMIC TABLES"):
        run(s, f"GRANT SELECT ON ALL {obj} IN SCHEMA {C.PROD_DATABASE}.{C.FEATURE_STORE_SCHEMA} TO ROLE {C.DEV_ROLE}")
        run(s, f"GRANT SELECT ON FUTURE {obj} IN SCHEMA {C.PROD_DATABASE}.{C.FEATURE_STORE_SCHEMA} TO ROLE {C.DEV_ROLE}")
